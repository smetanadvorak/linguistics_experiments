...
class FrameExtractor:
    ...
    def extract_regions(self, frame: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        for name, config in self.regions.items():
            ...
            top = config.top if config.top >= 1 else int(height * config.top)
            bottom = config.bottom if config.bottom >= 1 else int(height * config.bottom)
            left = config.left if config.left >= 1 else int(width * config.left)
            right = config.right if config.right >= 1 else int(width * config.right)
        ...
        for _, row in df.iterrows():
            ...
            if ret:
                ...
                for region_name, region_img in regions.items():
                    output_path = os.path.join(images_dir, f"{item_name}_{region_name}.jpg")
